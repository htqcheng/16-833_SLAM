...
def init_landmarks(init_measure, init_measure_cov, init_pose, init_pose_cov):
    ...
    landmark[::2] = x + init_measure[1::2] * np.cos(theta + init_measure[0::2])
    landmark[1::2] = y + init_measure[1::2] * np.sin(theta + init_measure[0::2])

    # Error covariance
    e = np.zeros((3 + 2 * k, 3+ 2 * k))
    e[0:3, 0:3] = init_pose_cov

    # Error Jacobian
    L = np.zeros((2*k, 3+2*k))
    for i in range(k):
        # Making my life easier
        beta = init_measure[2*i]
        r = init_measure[2*i+1]

        # Fill in ex, ey, etheta
        L[2*i:2*i+2, 0:2] = np.eye(2)
        L[2*i, 2] = -r*np.sin(beta)
        L[2*i+1, 2] = r*np.cos(beta)

        # Fill in e_beta, e_r
        L[2*i:2*i+2, 3+2*i] = np.array([-r*np.sin(theta+beta), r*np.cos(theta+beta)]).flatten()
        L[2*i:2*i+2, 4+2*i] = np.array([np.cos(theta+beta), np.sin(theta+beta)]).flatten()

        e[(3+2*i):(5+2*i), (3+2*i):(5+2*i)] = init_measure_cov

    # Calculate Covariance
    landmark_cov = L @ e @ L.T
    # print(L)

    # landmark_cov = np.eye(2*k) * 0.01
...
def predict(X, P, control, control_cov, k):
    ...
    \param X State vector of shape (3 + 2k, 1) stacking pose and landmarks. (x, y, theta, lx, ly)
    ...
    # Linearized uncertainty prediction
    ...
    # Uncertainty Update
    P[0:3, 0:3] = G @ P[0:3, 0:3] @ G.T + L @ R @ L.T

    # Mean Update; X = g(X, control)
    X[0] += d * np.cos(theta)
    X[1] += d * np.sin(theta)
    X[2] = warp2pi(theta + alpha)
    return X, P
...
def update(X_pre, P_pre, measure, measure_cov, k):
    ...
    \param measure Measurement signal of shape (2k, 1). [beta_i, r_i]
    ...
    lx = X_pre[3::2]
    ly = X_pre[4::2]
    print('x:', x, ' y:', y, ' theta:', theta, ' lx:', lx[0], ' ly:', ly[0])

    # Measurement Covariances
    Q = np.zeros((2*k, 2*k))
    # Jacobian matrix where states are x, y, z, [lx, ly](repeated k times)
    H = np.zeros((2*k, 3+2*k))
    # Expected measurement based on belief of location
    h_x_pre = np.zeros((2*k, 1))

    # Fill in the matrices
    for i in range(k):
        # x, y, theta
        c1 = (lx[i]-x)**2 + (ly[i]-y)**2
        H[2*i, 0] = (ly[i] - y)/c1
        H[2*i, 1] = (x - lx[i])/c1
        H[2*i, 2] = -1
        H[2*i+1, 0] = (x - lx[i])/np.sqrt(c1)
        H[2*i+1, 1] = (y - ly[i])/np.sqrt(c1)

        # landmarks
        H[2*i, 3+2*i] = (y - ly[i])/c1
        H[2*i, 4+2*i] = (lx[i] - x)/c1
        H[2*i+1, 3+2*i] = (lx[i] - x)/np.sqrt(c1)
        H[2*i+1, 4+2*i] = (ly[i] - y)/np.sqrt(c1)

        # Expected measurements
        h_x_pre[2*i] = warp2pi(np.arctan2((ly[i]-y), (lx[i]-x)) - theta)
        h_x_pre[2*i+1] = np.sqrt(c1)
    ...
    # print("Measurement Diff.: ", measure - h_x_pre)

    X_ret = X_pre + K @ (measure - h_x_pre)
    # print(X_ret[2])
    P_ret = (np.eye(3+2*k) - K @ H) @ P_pre
...
def evaluate(X, P, k):
    ...
    # Error calculation
    lx_est = X[3::2].flatten()
    ly_est = X[4::2].flatten()
    lx_true = l_true[::2]
    ly_true = l_true[1::2]
    e_tot = np.array([lx_true-lx_est, ly_true-ly_est])

    # Distances
    l_euclidean = np.zeros(k)
    l_mahalanobis = np.zeros(k)
    for i in range(k):
        e = e_tot[:, i]
        l_euclidean[i] = np.sqrt(e.T @ e)
        p_i = P[3+2*i : 5+2*i, 3+2*i : 5+2*i]
        l_mahalanobis[i] = np.sqrt(e.T @ np.linalg.inv(p_i) @ e)
    print("Euclidean: ", l_euclidean)
    print("Mahalanobis: ", l_mahalanobis)
